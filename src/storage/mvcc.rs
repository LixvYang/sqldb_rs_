...
impl<E: Engine> MvccTransaction<E> {
    ...
    pub fn commit(&self) -> Result<()> {
        ...
        //找到当前事务的TxnWrite
        let mut iter = eng.scan_prefix(MvccKeyPrefix::TxnWrite(self.state.version).encode());
        ...
        // 从活跃事务列表中删掉
        eng.delete(MvccKey::TxnActive(self.state.version).encode())?;
    ...
    fn write_inner(&self, key: Vec<u8>, value: Option<Vec<u8>>) -> Result<()> {
        ...
        if let Some((k, _)) = eng.scan(from..=to).last().transpose()? {
    ...
    // 扫描当前活跃的事务列表
    pub fn get_active_txns(engine: &mut MutexGuard<E>) -> Result<HashSet<Version>> {
